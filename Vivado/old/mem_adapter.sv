...
module mem_adapter #(
    ...
    parameter int PACK       = 128, // Relacionado a factor de ARRAY_RESHAPE en HLS
    parameter int ADDR_BITS  = 3    // log2(1024/128) = 3
)(
    input  logic                    clk,
    input  logic                    rst,
    input  logic [ELEM_BITS-1:0]    A_flat [N-1:0],  // Memorias planas (1024 x 10 bits)
        ...
        if (rst) begin
            ...
            if (A_ce0) begin
                for (i = 0; i < PACK; i++) begin
                    A_q0[i*ELEM_BITS +: ELEM_BITS] <= A_flat[A_address0*PACK + i];
                end
            end
            ...
            if (B_ce0) begin
                for (i = 0; i < PACK; i++) begin
                    B_q0[i*ELEM_BITS +: ELEM_BITS] <= B_flat[B_address0*PACK + i];
                end
            end
        end
    end
